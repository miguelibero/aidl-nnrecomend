...
def test_dataset():
    ...
    assert len(data.dataset) == 5*99057
    assert (data.dataset.idrange == (943, 2625)).all()
    assert data.matrix.shape == (2625, 2625)
    assert len(data.testset) == 943*100
    assert (data.testset.idrange == (943, 2595)).all()
    assert (data.dataset[50*5] == (0, 113+943, 1, 875072173)).all()
    assert (data.dataset[90000*5] == (861, 1008+943, 1, 879303622)).all()
